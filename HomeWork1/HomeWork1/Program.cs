...
internal class Program
{
    static void Main(string[] args)
    {
        while (true)
        {
            ...
            EditDistance.WriteDistance(str1, str2);
            Console.WriteLine();
        }
    }
}
...
internal static class EditDistance
{
    public static int Distance(string str1Param, string str2Param)
    {
        ...
        // str1 должна быть длиннее или равна str2
        if (str1Len < str2Len)
        {
            (str1Param, str2Param) = (str2Param, str1Param);
        ...
        for (int i = 1; i <= str1Len; i++)
        {
            for (int j = 1; j <= str2Len; j++)
            {
                ...
                int subst = matrix[i - 1, j - 1] + symbEqual; // замена или совпадение

                matrix[i, j] = Math.Min(Math.Min(ins, del), subst);

                // транспозиция (поправка дамерау)
                if ((i > 1) && (j > 1) &&
